    ...
    def to_rpn(self, input):
        ...
        for token in tokens:
            ...
            if Validator.is_numeric(token):
                ...
                if Validator.is_open_brace(token):
                    operator_stack.push(token)
                    ...
                    if this_priority > head_priority():
                        operator_stack.push(token)
                    else:
                        while operator_stack.not_empty() and head_priority() >= this_priority: output.append(
                            operator_stack.pop())
                        operator_stack.push(token)
